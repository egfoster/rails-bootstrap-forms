...
class BootstrapFormTest < ActionView::TestCase
  ...
  test "help is preserved when inline_errors: false is passed to bootstrap_form_for" do
    ...
    expected = %{<form accept-charset="UTF-8" action="/users" class="new_user" id="new_user" method="post" role="form"><div style="margin:0;padding:0;display:inline"><input name="utf8" type="hidden" value="&#x2713;" /></div><div class="form-group has-danger"><label class="control-label required" for="user_email">Email</label><input class="form-control" id="user_email" name="user[email]" type="text" /><small class="text-help">This is required</small></div></form>}
    assert_equal expected, output
  end
